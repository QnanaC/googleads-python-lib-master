...
ofile = open('writetest.csv', "wb")
writer = csv.writer(ofile, delimiter=';')
writer.writerow(['Id', 'Name','Display Name', 'Custom Targeting Key Id'])

def main(client):
  # Initialize appropriate service.
  custom_targeting_service = client.GetService(
      'CustomTargetingService', version='v201705')

  # Create statement to get targeting keys by id or name (value).
  # query = "WHERE id IN (316224, 248184)"
  query = "WHERE name IN ('ingredients')"
  targeting_key_statement = dfp.FilterStatement(query)
    ...
    while True:
      ...
      if 'results' in response:
        for custom_targeting_value in response['results']:
          ...
          valuestofile = [custom_targeting_value['id'], custom_targeting_value['name'],
                          custom_targeting_value['displayName'],
                          custom_targeting_value['customTargetingKeyId']]
